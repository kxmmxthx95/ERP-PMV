/** Server-side horoscope fetch (Aztro → fallback). Used by Vite dev proxy and Cloud Function. */
...
const COMPAT = ['aries', 'taurus', 'gemini', 'cancer', 'leo', 'virgo', 'libra', 'scorpio', 'sagittarius', 'capricorn', 'aquarius', 'pisces'];
...
export function parseHoroscopeQuery(signRaw: string, dayRaw: string): { sign: string; day: HoroscopeDay } | null {
  const sign = signRaw.toLowerCase();
  const day = dayRaw.toLowerCase() as HoroscopeDay;
  if (!VALID_SIGNS.has(sign) || !VALID_DAYS.has(day)) return null;
  return { sign, day };
}

export async function fetchHoroscope(sign: string, day: HoroscopeDay): Promise<HoroscopePayload> {
  try {
