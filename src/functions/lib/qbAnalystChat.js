...
function resolveGeminiApiKey() {
    ...
    if (fromEnv)
        return fromEnv;
    throw new https_1.HttpsError("failed-precondition", "ยังไม่ได้ตั้งค่า GEMINI_API_KEY — ติดต่อผู้ดูแลระบบ");
}
async function sampleQuestionStats(setId) {
...
async function buildQuestionBankContext() {
    ...
    const lines = summaries.map((s) => {
        ...
        const ind = s.indicators.length > 0 ? ` | ตัวชี้วัด: ${s.indicators.join(", ")}` : "";
...
async function callGeminiOnce(apiKey, model, systemPrompt, history, userMessage) {
    const contents = [
        ...history.map((turn) => ({
            role: turn.role,
...
async function callGemini(apiKey, systemPrompt, history, userMessage) {
    ...
    if (lastError?.code === 429 || lastError?.status === 429) {
        throw new https_1.HttpsError("resource-exhausted", "Gemini ใช้งานเกินโควต้า กรุณาลองใหม่ภายหลัง หรือตรวจสอบแผน/API key ที่ Google AI Studio");
    }
    throw new https_1.HttpsError("internal", "Gemini ตอบกลับไม่สำเร็จ กรุณาลองใหม่");
}
exports.qbAnalystChat = (0, https_1.onCall)({
    ...
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "กรุณาเข้าสู่ระบบก่อนใช้งาน QB Analyst");
    }
    const message = typeof request.data?.message === "string" ? request.data.message.trim() : "";
    if (!message) {
        throw new https_1.HttpsError("invalid-argument", "กรุณาพิมพ์ข้อความ");
    }
    if (message.length > 4000) {
        throw new https_1.HttpsError("invalid-argument", "ข้อความยาวเกินไป");
    }
    ...
    const history = rawHistory
        ...
        if (!trimmed)
            return null;
        return { role, text: trimmed.slice(0, 4000) };
    })
...
//# sourceMappingURL=qbAnalystChat.js.map