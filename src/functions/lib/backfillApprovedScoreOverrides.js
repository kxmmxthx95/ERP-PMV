...
function isLinkedToGradeBook(settings) {
    if (settings.scoreCollectionLinked === false)
        return false;
    if (settings.scoreCollectionEnabled === true)
        return true;
    if (settings.scoreCollectionEnabled === false)
        return false;
    return (settings.gradeBookSubjects?.length ?? 0) > 0 || !!settings.gradeBookSubjectId;
}
function scoreCollectionTypeToGradeField(type) {
    if (type === "midterm")
        return "midtermScore";
    if (type === "final")
        return "finalScore";
...
function rawPointsToPercent(raw, maxPoints) {
...
//# sourceMappingURL=backfillApprovedScoreOverrides.js.map