...
export function StudentExamScoreDetailDrawer({
  ...
  const [isSavingScoreEdit, setIsSavingScoreEdit] = useState(false);
  ...
  const saveScoreEdit = useCallback(async () => {
    if (!attempt || !user) return;
    if (editScoreInput.trim() === '') {
      toast.error('กรุณากรอกคะแนนใหม่');
      return;
    }
    const parsed = Number(editScoreInput);
    ...
    setIsSavingScoreEdit(true);
    try {
      await updateDoc(doc(db, 'exam_rooms', room.id, 'attempts', attempt.id), {
        score: parsed,
      ...
      toast.success('บันทึกคะแนนใหม่แล้ว');
      setShowEditScoreForm(false);
      setEditScoreInput('');
      setEditScoreReason('');
    } catch {
      ...
      setIsSavingScoreEdit(false);
    }
  }, [attempt, user, editScoreInput, editScoreReason, roundTotal, room.id, room.title, student, activeRound, attemptScore, currentUserName]);
                  ...
                  {showEditScoreForm ? (
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          type="button"
                          disabled={isSavingScoreEdit}
                          onClick={() => void saveScoreEdit()}
                        ...
                        >
                          {isSavingScoreEdit ? 'กำลังบันทึก...' : 'บันทึกคะแนน'}
                        </button>
                        <button
                          type="button"
                          onClick={() => setShowEditScoreForm(false)}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setEditScoreInput(attemptScore != null ? String(attemptScore) : '');
