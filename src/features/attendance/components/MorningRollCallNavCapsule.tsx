...
export type MorningRollCallNavTab = 'rollcall' | 'dashboard';

const MORNING_ROLL_CALL_TAB_CONFIG: Record<
  MorningRollCallNavTab,
  { label: string; icon: IconType; path: string }
> = {
  dashboard: {
    label: 'สรุป',
    icon: HiOutlineChartPie,
    path: '/portal/morning-rollcall',
  },
  rollcall: {
    label: 'เช็กชื่อ',
    icon: HiOutlineClipboardDocumentCheck,
    path: '/portal/morning-rollcall/check',
  },
};

const TABS = (
  Object.entries(MORNING_ROLL_CALL_TAB_CONFIG) as [
    MorningRollCallNavTab,
    (typeof MORNING_ROLL_CALL_TAB_CONFIG)[MorningRollCallNavTab],
  ][]
);

interface MorningRollCallNavMenuProps {
  active: MorningRollCallNavTab;
}

export function MorningRollCallNavMenu({ active }: MorningRollCallNavMenuProps) {
  ...
  const activeConfig = MORNING_ROLL_CALL_TAB_CONFIG[active];
  ...
  const mobileTabSelect = (
    <div className="pointer-events-auto relative flex min-w-0 max-w-[calc(100vw-112px)] items-center justify-center lg:hidden">
      <button
        ...
        aria-label="เลือกหน้าเช็กชื่อเข้าแถว"
...
/** @deprecated Use MorningRollCallNavMenu */
export function MorningRollCallNavCapsule(props: MorningRollCallNavMenuProps) {
  return <MorningRollCallNavMenu {...props} />;
}
