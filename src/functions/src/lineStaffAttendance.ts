...
import { findUserByLineId } from "./lineUserLookup";
...
async function verifyLineAccessToken(accessToken: string): Promise<LineProfile> {
  try {
    ...
    if (!userId) {
      throw new HttpsError("unauthenticated", "Invalid LINE profile");
    }
    const displayName =
      typeof res.data?.displayName === "string" && res.data.displayName.trim()
        ? res.data.displayName.trim()
        : "LINE User";
    return { userId, displayName };
  } catch (err) {
    if (axios.isAxiosError(err) && (err.response?.status === 401 || err.response?.status === 403)) {
      throw new HttpsError("unauthenticated", "LINE session expired — กรุณาเปิดใหม่จาก LINE");
    }
    console.error("[lineStaffAttendance] LINE profile error", err);
    throw new HttpsError("unavailable", "Cannot verify LINE account");
  }
}
...
export const lineStaffAttendance = onCall(
  {
    region: REGION,
    cors: [
  ...
  async (request) => {
    const accessToken =
      typeof request.data?.accessToken === "string" ? request.data.accessToken.trim() : "";
    const actionRaw = typeof request.data?.action === "string" ? request.data.action.trim() : "status";
    const action = actionRaw as LineAction;
    ...
    const user = await findUserByLineId(db, lineProfile.userId);
    ...
    if (!STAFF_ROLES.has(user.role)) {
      throw new HttpsError(
        "permission-denied",
        "ฟีเจอร์นี้สำหรับครูและบุคลากรเท่านั้น",
      );
    }
    ...
    if (holiday.isHoliday) {
      throw new HttpsError(
        "failed-precondition",
        `วันนี้เป็นวันหยุด (${holiday.holidayTitle ?? "วันหยุด"}) ไม่สามารถลงเวลาทำงานได้`,
      );
    }
    ...
    if (action === "checkIn") {
      ...
      if (distance > config.radiusMeters) {
        throw new HttpsError(
          "failed-precondition",
          `คุณอยู่นอกพื้นที่โรงเรียน (ห่าง ${Math.round(distance)} เมตร / รัศมี ${config.radiusMeters} เมตร)`,
        );
      }
