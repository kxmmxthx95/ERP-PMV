...
export default function SubjectGroupExamRoomDrawer({
  open,
  onClose,
  group,
  rooms,
  ...
  const groupRooms = useMemo(() => {
    if (!group) return [];
    return rooms
      .filter((room) => resolveSubjectGroupId(room) === group.groupKey)
      .sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0));
  }, [rooms, group]);

  const totalPages = Math.max(1, Math.ceil(groupRooms.length / ROOMS_PER_PAGE));
  ...
  const paginatedRooms = useMemo(() => {
    ...
    return groupRooms.slice(start, start + ROOMS_PER_PAGE);
  }, [groupRooms, page]);

  const rangeStart = groupRooms.length === 0 ? 0 : (page - 1) * ROOMS_PER_PAGE + 1;
  const rangeEnd = Math.min(page * ROOMS_PER_PAGE, groupRooms.length);
    ...
    <Drawer open={open} onOpenChange={(next) => !next && onClose()} direction="right">
      <DrawerContent className={DRAWER_CONTENT_CLASS}>
        <div className="flex h-full min-h-0 flex-col">
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                <div
                  ...
                  style={{ background: subjectIconGradient(group?.groupKey ?? 'other') }}
                >
                  <SubjectIcon subjectGroup={group?.groupKey ?? 'other'} size={22} />
                </div>
                <div className="min-w-0 text-left">
                  <DrawerTitle className="line-clamp-2 text-base font-black text-slate-900">
                    {group?.name ?? 'กลุ่มสาระวิชา'}
                  </DrawerTitle>
                  <DrawerDescription className="text-xs font-semibold text-slate-500">
                    ห้องสอบในกลุ่มสาระ {groupRooms.length} ห้อง
                  </DrawerDescription>
                </div>
              </div>
              <DrawerClose asChild>
          ...
          {groupRooms.length > 0 && totalPages > 1 && (
            <div className="shrink-0 border-t border-slate-100 px-5 py-3">
              <div className="flex flex-col items-center gap-2 sm:flex-row sm:justify-between">
                <p className="text-[11px] font-semibold text-slate-400">
                  แสดง {rangeStart}–{rangeEnd} จาก {groupRooms.length} ห้อง
                </p>
