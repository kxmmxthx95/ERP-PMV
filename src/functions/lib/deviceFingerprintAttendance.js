...
exports.hashDeviceApiKey = exports.deviceFingerprintAttendance = void 0;
...
function resolveUserListCategory(role, position) {
    if (role === "student")
        return "student";
    if (role === "teacher") {
        const pos = (position ?? "").trim();
        if (pos === "ครูพิเศษ" || pos.includes("พิเศษ"))
...
async function findUserByFingerprintTemplateId(templateId) {
    const snap = await db
        .collection("users")
    ...
    for (const docSnap of snap.docs) {
        ...
        if (status !== "active")
            continue;
        const role = (0, lineUserLookup_1.resolveUserRole)(data);
...
async function verifyDevice(deviceId, apiKey) {
 ...
 */
exports.deviceFingerprintAttendance = (0, https_1.onRequest)({
    region: REGION,
    cors: true,
    invoker: "public",
}, async (req, res) => {
    ...
    try {
        ...
        if (action === "listUsers") {
            ...
            const users = snap.docs
                ...
                if (typeof templateId !== "number" || templateId < 1 || templateId > 127)
                    return null;
                const role = (0, lineUserLookup_1.resolveUserRole)(data);
                if (!role)
                    return null;
                const position = teacherPositions.get(docSnap.id) ??
                    (typeof data.teacherId === "string" ? teacherPositions.get(data.teacherId) : undefined) ??
                    "";
                const code = typeof data.studentCode === "string" && data.studentCode.trim()
        ...
        const fingerprintTemplateId = typeof templateRaw === "number"
            ? templateRaw
...
exports.hashDeviceApiKey = hashDeviceApiKey;
//# sourceMappingURL=deviceFingerprintAttendance.js.map